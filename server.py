...
def init_defaults():
    ...
    session.setdefault("cart_count", 0)
...
def register():
    ...
    if request.method == "POST":
        ...
        for user in ALLOWED_USERS:
            ...
            ALLOWED_USERS.append({"user": username, "pass": scripts.hash_password(password)})
    ...
    # (either due to invalid input or username already exists)
    # Show the registration form again with an error message
...
def cart():
    ...
    return render_template("cart.html", cart_items=cart_items, current_page="cart")
